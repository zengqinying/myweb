...
export default function VideoGallery({ compact = false }) {
  ...
  // IntersectionObserver: detect when the gallery enters the viewport
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // First video stays as static cover until the gallery scrolls into view
        ...
        <div className="gallery-nav">
          ...
          <div className="gallery-dots">
            {VIDEOS.map((_, i) => (
              <button
                key={i}
                className={"gallery-dot" + (i === currentIdx ? " active" : "")}
                onClick={() => jumpTo(i)}
                title={"第 " + (i + 1) + " 页"}
              />
            ))}
